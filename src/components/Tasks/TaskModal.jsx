...
const TaskModal = ({ onSubmit, onModalClose }) => {
    const { task, editDispatch } = useContext(TaskToEditContext);
    const [isAdd, setIsAdd] = useState(Object.is(task, null));
    ...
    const { title, description, tags, priority } = state;
    ...
    const handleSubmit = (e) => {
        ...
        if (!title && !description && !priority && tags.length === 0) {
            ...
            toast.success(
                `${state.title} has been ${
                    isAdd ? "added" : "edited"
                } to the task list.`
            );

            onSubmit(state, isAdd);
        }
    };
